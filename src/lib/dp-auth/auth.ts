...
export class Auth {
	...
	async handleProviderCallback(event: RequestEvent, provider: Provider): Promise<any> {
	...
	async handleEndpoint(event: RequestEvent): Promise<any> {
	...
	getSession: any = async (event: RequestEvent) => {
		const { request } = event;
